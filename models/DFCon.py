...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.DFCon = configs.DFCon
        self.DFCon_wnorm = configs.DFCon_wnorm
        self.DFCon_multiscales = configs.DFCon_multiscales
        self.decomp_top_k = configs.decomp_top_k
        ...
        self.ch_mlps = nn.ModuleList([nn.Linear(self.repr_dims, self.c_out) for _ in self.DFCon_multiscales])
        self.length_mlp = nn.Linear(self.seq_len, self.pred_len)
        self.s_dm_mlp = nn.Linear(self.repr_dims, self.c_out)
        if configs.DF_decomp == 'mv':
            self.trend_decoms = nn.ModuleList(
                [series_decomp(kernel_size=dlen + 1) for dlen in self.DFCon_multiscales])
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        elif self.DFCon_wnorm == 'Mean':
            seq_mean = x_enc.mean(dim=1, keepdim=True).detach()
            short_x = (x_enc - seq_mean)
            long_x = short_x.clone()
        elif self.DFCon_wnorm == 'Decomp':
            short_x, long_x = self.input_decom(x_enc)
        elif self.DFCon_wnorm == 'LastVal':
            seq_last = x_enc[:, -1:, :].detach()
            short_x = (x_enc - seq_last)
            long_x = short_x.clone()
        elif self.DFCon_wnorm == 'FFT':
            short_x, long_x = self.fft_decomp(x_enc)
        else:
            raise Exception(
                f'Not Supported Window Normalization:{self.DFCon_wnorm}. Use {{"ReVIN | Mean | LastVal | Decomp"}}.')
