...
class RidgeRegressor(nn.Module):
    ...
    def get_weights(self, X: Tensor, Y: Tensor, reg_coeff: float) -> Tensor:
        ...
        if n_samples >= n_dim:
            # standard
            A = torch.bmm(X.transpose(1,-1), X)
            A.diagonal(dim1=-2, dim2=-1).add_(reg_coeff)
            B = torch.bmm(X.transpose(1,-1), Y)
            weights = torch.linalg.solve(A, B)
            # weights = torch.solve(A, B)
        else:
            # Woodbury
            A = torch.bmm(X, X.transpose(1,-1))
            A.diagonal(dim1=-2, dim2=-1).add_(reg_coeff)
            weights = torch.bmm(X.transpose(1,-1), torch.linalg.solve(A, Y))
